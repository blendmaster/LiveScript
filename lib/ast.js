...
exports.Literal = Literal = (function(superclass){
  ...
  prototype.compile = function(o, level){
    ...
    case 'void':
      if (level === LEVEL_TOP) {
        return '';
      } else if (level === LEVEL_CALL) {
        return this.carp('invalid use of ' + this.value);
      } else {
        return 'void 8';
      }
      break;
...
exports.Var = Var = (function(superclass){
  ...
  function Var(value){
    ...
    if (/\s/.test(this$.value)) {
      console.error("probably not a var: " + this$.value);
    }
    return this$;
  } function ctor$(){} ctor$.prototype = prototype;
...
exports.Chain = Chain = (function(superclass){
  ...
  prototype.add = function(it){
    var last, ref$, ref1$, index, bi, that, logics, call, f;
    if (this.tails.length) {
      last = (ref$ = this.tails)[ref$.length - 1];
      if (last instanceof Call && ((ref1$ = last.partialized) != null ? ref1$.length : void 8) === 1 && it.args.length === 1) {
    ...
    } else if (that = (ref1$ = it.vivify, delete it.vivify, ref1$)) {
  ...
  prototype.cacheReference = function(o){
    var name, ref$, base, ref, bref, ref1$, nref;
    ...
    if (base.isComplex()) {
      ...
      bref = (ref1$ = Var(ref), ref1$.temp = true, ref1$);
    }
    if (!name) {
      return [base, bref];
    }
    if (name.isComplex()) {
      ...
      nref = Index((ref1$ = Var(ref), ref1$.temp = true, ref1$));
    }
    ...
    for (i$ = 0, len$ = (ref$ = this.tails).length; i$ < len$; ++i$) {
      ...
      if (ref1$ = node.soak, delete node.soak, ref1$) {
        ...
        test = node instanceof Call
          ? (ref1$ = bust.cacheReference(o), test = ref1$[0], this.head = ref1$[1], Binary('===', Unary('typeof', test), Literal("'function'")))
          : (i && node.assign
  ...
  prototype.expandSplat = function(o){
    var tails, i, call, args, ctx, seen, i$, len$, node, argsExpr, cachedTails, ref$;
    tails = (this).tails;
    i = -1;
    while (call = tails[++i]) {
      if (!call.args) {
      ...
      ctx = call.method === '.call' ? args.shift() : void 8;
      seen = false;
      ...
      argsExpr = Splat.expandArray(args, true);
      if (call['new']) {
        this.splattedNewArgs = argsExpr;
      } else {
        if (!ctx && tails[i - 1] instanceof Index) {
          cachedTails = tails.splice(0, i - 1);
          ref$ = Chain(this.head, cachedTails).cache(o, true), this.head = ref$[0], ctx = ref$[1];
          i = 0;
        }
        call.method = '.apply';
        call.args = [ctx || Literal('null'), argsExpr];
      }
    }
  };
  ...
  prototype.expandStar = function(o){
    ...
    while (that = tails[++i]) {
      ...
      this.head = Chain(sub, [tails.shift()]);
      if (temps) {
...
exports.Obj = Obj = (function(superclass){
  ...
  prototype.toSlice = function(o, base, symbol, assign){
    var items, ref$, ref, temps, i$, len$, i, node, name, chain, logic, ref1$, key, val;
    items = (this).items;
    if (items.length > 1) {
    ...
    for (i$ = 0, len$ = items.length; i$ < len$; ++i$) {
      ...
      if (node instanceof Prop || node instanceof Splat) {
        ...
        if (node instanceof Parens) {
          ref1$ = node.cache(o, true), key = ref1$[0], node = ref1$[1];
          if (assign) {
            ref1$ = [node, key], key = ref1$[0], node = ref1$[1];
          }
          key = Parens(key);
        } else {
          key = node;
  ...
  prototype.compileNode = function(o){
    var items, toCompile, i$, len$, i, node, rest, ref$, code, idt, dic, logic, multi, key, val;
    items = (this).items;
    if (!(items.length > 0)) {
      return this.front ? '({})' : '{}';
    }
    ...
    if (rest) {
      return Import((ref$ = Obj(toCompile), ref$.front = this.front, ref$), Obj(rest), false, true).compile((o.indent = this.tab, o));
    } else {
      code = '';
      idt = '\n' + (o.indent += TAB);
      dic = {};
      for (i$ = 0, len$ = toCompile.length; i$ < len$; ++i$) {
        node = toCompile[i$];
        if (logic = node.getDefault()) {
          node = node.first;
        }
...
exports.Binary = Binary = (function(superclass){
  ...
  prototype.compileMethod = function(o, klass, method, arg){
    ...
    if (this.first["is" + klass]()) {
      ...
      return (x$ = Chain(Var(util(method))), x$.add(Index(Key('call'))), x$.add(Call(args)), x$).compile(o);
    }
  };
  ...
  prototype.compileRepeat = function(o){
    ...
    if (items) {
      if (n < 1) {
        return Block(items).add(Arr([])).compile(o);
      }
      refs = [];
      for (i$ = 0, len$ = items.length; i$ < len$; ++i$) {
        i = i$;
  ...
  prototype.compilePow = function(o){
    var x$;
    return (x$ = Chain(Var('Math')), x$.add(Index(Key('pow'))), x$.add(Call([this.first, this.second])), x$).compile(o);
  };
...
exports.Assign = Assign = (function(superclass){
  ...
  prototype.compileDestructuring = function(o, left){
    var items, len, ret, rite, rCompileLevel, that, cache, isLiteral, isVar, assignedByLeft, rref, list, code;
    items = left.items, len = items.length;
    ret = o.level && !this['void'];
    rite = this.right;
    rCompileLevel = len === 1 ? LEVEL_CALL : LEVEL_LIST;
    if (that = left.name) {
      cache = that + " = " + rite.compile(o, rCompileLevel);
      o.scope.declare(left.name, left);
      rite = Var(left.name);
    } else if (ret || len > 1) {
      isLiteral = rite instanceof Literal;
      isVar = rite instanceof Var;
      assignedByLeft = isVar && left.assigns(rite.value);
      if (!isLiteral && !isVar || assignedByLeft) {
        rref = o.scope.temporary();
        cache = rref + " = " + rite.compile(o, rCompileLevel);
        rite = Var(rref);
      }
    }
    ...
    if (ret || !list.length) {
      list.push(rite.compile(o, rCompileLevel));
    }
    code = list.join(', ');
  ...
  prototype.rendArr = function(o, nodes, rite){
    var i$, len$, i, node, skip, len, args, x$, slice, val, ivar, start, f, idx, ref$, results$ = [];
    for (i$ = 0, len$ = nodes.length; i$ < len$; ++i$) {
      ...
      if (node instanceof Splat) {
        ...
        if (i + 1 === len) {
          ...
          x$ = slice = Chain(Var(util('slice')));
          x$.add(Index(Key('call')));
          x$.add(Call(args));
          val = Arr.wrap(slice);
        } else {
          val = ivar = Binary('-', Chain(rite).add(Index(Key('length'))), Literal((len - i - 1) + ""));
          if (skip && i + 2 === len) {
            continue;
          }
          start = i + 1;
          ivar = Var(o.scope.temporary('i'));
          this.temps = [ivar.value];
          val = skip
            ? (node = ivar, val)
            : (f = If(Binary('<', Literal(i), Assign(ivar, val))), f.compact = true, f.then = Chain(Var(util('slice'))).add(Index(Key('call'))).add(Call([rite, Literal(i), ivar])), f['else'] = Block([Assign(ivar, Literal(i)), Arr([])]), Arr.wrap(f));
        }
      } else {
        idx = ivar != null
          ? start < i ? Binary('+', ivar, Literal((i - start) + "")) : ivar
          : Literal(i);
  ...
  prototype.rendObj = function(o, nodes, rite){
    var i$, len$, node, splat, logic, ref$, key, val, results$ = [];
    for (i$ = 0, len$ = nodes.length; i$ < len$; ++i$) {
      node = nodes[i$];
      ...
      if (!(rite instanceof Var)) {
        rite = Parens(rite, true);
      }
...
exports.Import = Import = (function(superclass){
  ...
  function Import(left, right, all, forceImport){
    var this$ = this instanceof ctor$ ? this : new ctor$;
    this$.left = left;
    this$.right = right;
    this$.all = all && 'All';
    forceImport == null && (forceImport = false);
    ...
    if (left instanceof Existence && !left.negated) {
      if ((left = left.it) instanceof Var) {
        ...
        if (!o.scope.check(value, true)) {
          left = Binary('&&', Binary('!=', Unary('typeof', left), Literal("'undefined'")), left);
        }
      } else {
...
exports.In = In = (function(superclass){
  ...
  prototype.compileNode = function(o){
    var items, array, code, ref$, sub, ref, cmp, cnj, i$, len$, i, test;
  ...
  prototype.compileNode = function(o){
    ...
    if (this.bound === 'this$') {
      if (this.ctor) {
        ...
        body.lines.push(Return(Var('this$')));
...
exports.Class = Class = (function(superclass){
  ...
  prototype.compile = function(o, level){
    ...
    if (that = this.sup) {
      ...
      fun.proto = Util.Extends(fun.cname ? Block([Assign(imports.add(Index(Key('displayName'))), Literal("'" + name + "'")), Var(name)]) : imports, (ref$ = fun.params)[ref$.length] = Var('superclass'));
    }
    if (that = this.mixins) {
      res$ = [];
      for (i$ = 0, len$ = that.length; i$ < len$; ++i$) {
        args[args.length] = that[i$];
        res$.push(Import(proto, Chain(Var('arguments')).add(Index(Literal(args.length - 1))), true));
      }
      imports = res$;
      body.prepend.apply(body, imports);
    }
    if (fun.cname && !this.sup) {
      body.prepend(Assign(Chain(Var(name)).add(Index(Key('displayName'))), Literal("'" + name + "'")));
    }
...
exports.Splat = Splat = (function(superclass){
  ...
  Splat.expandArray = function(list, apply){
    var index, i$, len$, node, args, atoms, ref$, head, x$;
    expand(list);
    index = 0;
    for (i$ = 0, len$ = list.length; i$ < len$; ++i$) {
      node = list[i$];
...
exports.YadaYadaYada = YadaYadaYada = (function(superclass){
  var prototype = extend$((import$(YadaYadaYada, superclass).displayName = 'YadaYadaYada', YadaYadaYada), superclass).prototype, constructor = YadaYadaYada;
  function YadaYadaYada(){
    var this$ = this instanceof ctor$ ? this : new ctor$;
    return this$;
  } function ctor$(){} ctor$.prototype = prototype;
  prototype.compileNode = function(o){
    return "throw Error('unimplemented');";
  };
  return YadaYadaYada;
}(Throw));
...
exports.For = For = (function(superclass){
  ...
  prototype.addBody = function(body){
    ...
    if (this['let']) {
      ...
      body = Block(Call['let']((x$ = [], (that = this.index) && x$.push(Assign(Var(that), Var('index$$'))), (that = this.item) && x$.push(Assign(that, Var('item$$'))), x$), body));
    }
  ...
  prototype.compileNode = function(o){
    ...
    if (this.index && !this.object) {
      head += '\n' + o.indent + Assign(Var(this.index), Var(idx)).compile(o, LEVEL_TOP) + ';';
    }
    if (this.item && !this.item.isEmpty()) {
      head += '\n' + o.indent;
      head += Assign(this.item, Chain(Var(svar)).add(Index(Var(idx)))).compile(o, LEVEL_TOP);
      head += ';';
    }
    if (this.ref) {
...
exports.Try = Try = (function(superclass){
  ...
  prototype.compileNode = function(o){
    ...
    if (this.recovery != null || !this.ensure) {
      if (this.recovery == null) {
        this.recovery = Block([]);
      }
      code += ' catch (e$) ' + this.compileBlock(o, this.recovery);
    }
    if (that = this.ensure) {
...
exports.Case = Case = (function(superclass){
  ...
  prototype.compileCase = function(o, tab, nobr, bool, type, target){
    ...
    ft = (last != null ? last.value : void 8) === 'fallthrough';
    if (ft) {
      lines.pop();
    }
    o.indent = tab += TAB;
...
exports.If = If = (function(superclass){
  ...
  prototype.compileExpression = function(o){
    ...
    pad = !this.compact && els.isComplex() ? '\n' + (o.indent += TAB) : ' ';
...
exports.JS = JS = (function(superclass){
  ...
  prototype.compile = function(it){
    console.error("JS used internally:" + (this.comment ? '(comment)' : '') + " " + this.code);
    if (this.literal) {
