...
exports.doID = function(code, index){
  var input, regexMatch, id, e, last, ref$, tag, ref1$, ref2$, ref3$, that, ref4$, ref5$;
  ...
  case 'instanceof':
    ...
    tag = (ref2$ = this.tokens)[ref2$.length - 1][0] === '(' ? 'BIOPR' : 'RELATION';
    break;
  case 'not':
  ...
  default:
    ...
    if (!last[1] && ((ref3$ = last[0]) === 'FUNCTION' || ref3$ === 'LABEL')) {
      last[1] = id;
      last.spaced = false;
      return input.length;
    }
    ...
    case 'til':
      ...
      if (this.fget('from')) {
        ...
        this.fset('by', true);
        tag = 'TO';
      } else if (!last.callable && last[0] === 'STRNUM' && (ref3$ = this.tokens)[ref3$.length - 2][0] === '[') {
        last[0] = 'RANGE';
        last.op = id;
        return id.length;
      ...
      break;
    case 'by':
      if (last[0] === 'STRNUM' && (ref4$ = this.tokens)[ref4$.length - 2][0] === 'RANGE' && (ref5$ = this.tokens)[ref5$.length - 3][0] === '[') {
        tag = 'RANGE_BY';
      } else if (in$(']', this.closes)) {
        tag = 'BY';
...
exports.doComment = function(code, index){
  var comment, end, ref$;
  this.carp('Block comments are not supported');
...
exports.doJS = function(code, lastIndex){
  ...
  this.carp('embedded JS is not supported');
...
exports.doLiteral = function(code, index){
  ...
  case ']':
  case ')':
    if (tag === ')' && ((ref1$ = this.last[0]) === '+-' || ref1$ === 'COMPARE' || ref1$ === 'LOGIC' || ref1$ === 'MATH' || ref1$ === 'POWER' || ref1$ === 'SHIFT' || ref1$ === 'BITWISE' || ref1$ === 'CONCAT' || ref1$ === 'COMPOSE' || ref1$ === 'RELATION' || ref1$ === 'PIPE' || ref1$ === 'BACKPIPE' || ref1$ === 'IMPORT' || ref1$ === 'CLONEPORT' || ref1$ === 'ASSIGN')) {
      (ref1$ = this.tokens)[ref1$.length - 1][0] = (function(){
        switch (this.last[0]) {
        case 'RELATION':
          return 'BIOPR';
        case 'PIPE':
  ...
  case '|>=':
    ...
    } else if ((val === '+=' || val === '-=') && !able(this.tokens) && ((ref2$ = this.last[0]) !== '+-' && ref2$ !== 'UNARY' && ref2$ !== 'LABEL')) {
  ...
  case '::=':
    ...
    return sym.length;
  case '*':
    if (that = ((ref2$ = this.last[0]) === 'NEWLINE' || ref2$ === 'INDENT' || ref2$ === 'THEN' || ref2$ === '=>') && (INLINEDENT.lastIndex = index + 1, INLINEDENT).exec(code)[0].length) {
  ...
  case '@':
    this.adi();
    if (this.last[0] === 'DOT' && this.last[1] === '.' && (ref2$ = this.tokens)[ref2$.length - 2][0] === 'ID' && (ref3$ = this.tokens)[ref3$.length - 2][1] === 'constructor') {
      this.tokens.pop();
      this.tokens.pop();
